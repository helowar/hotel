 ...
 * 
 */
public class LogClauseTemplateHandle {
	
	private static final MyLog log = MyLog.getLogger(LogClauseTemplateHandle.class);
     ...
     * @param jp
     */
    public void  logClauseTemplate(JoinPoint jp) {
    	
    	Object[] objLog =  jp.getArgs();
    	
        HtlPreconcertItemTemplet htlPreconcertItemTemplet = new HtlPreconcertItemTemplet();

        htlPreconcertItemTemplet = (HtlPreconcertItemTemplet) objLog[0];
        
        String methodStr =jp.getSignature().getName();
    	
    	log.info("=====================酒店预定担保条款模板日志的写入=====Aop method : "+methodStr);
        ...
        if (null != htlPreconcertItemTemplet) {
            ...
            if (htlPreconcertItemTemplet.getActive().equals(BizRuleCheck.getFalseString())) {
                ...
                HtlPreconcertItemTemplet htlPreconItTplt = (HtlPreconcertItemTemplet) entityManager.find(
