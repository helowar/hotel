...
public class HotelCheckAction extends GenericWebAction {
	 ...
	 */
    // add by diandian.hou
    private String currencyValueForShow;
    private String rate_RMBToHK;
	public String getRate_RMBToHK() {
		return rate_RMBToHK;
	}
	private String rate_MOPToHK;
	public String getRate_MOPToHK() {
		double rate_MOPToHKD = getRate_MOPToHKD();
		rate_MOPToHK = String.valueOf(rate_MOPToHKD);
		return rate_MOPToHK;
	}
	
	public String execute() {
		Map params = super.getParams();
//	   System.out.println("fffff:"+params);
		//得到支付方式来判断是rmb支付还是hk支付
...
//			
//		}
		//给rate_MOPToHK赋值,界面调用 add by diandian.hou
		getRate_MOPToHK();
		...
		// add by diandian.hou 2010-8-19
		if("pre_pay".equals(params.get("payMethod")) || order.isPayToPrepay()){
			currencyValueForShow = "HK$";
		}else{
			currencyValueForShow ="￥";
		}
		rate_RMBToHK=String.valueOf( 1 / (Double.parseDouble(CurrencyBean.rateMap.get(
...
//		hotelOrderFromBean.setNeedAssure(true);
		//订单来源为繁体web,add by diandian.hou 
		setOrderSourceForFantiWeb(order,OrderSource.FAN_TI_NET);
        return SUCCESS;
	}
	
	////设置order的source add by diandian.hou 2010-8-26
...
//                hotelCheckOrderService.calculateSuretyAmount(assureDetailString, hotelOrderFromBean, order, reservation);
	...
	public String getCurrencyValueForShow() {
		return currencyValueForShow;
	}
