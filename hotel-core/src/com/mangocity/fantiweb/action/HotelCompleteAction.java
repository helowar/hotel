...
public class HotelCompleteAction extends GenericWebAction {
	...
	public String execute() {
		...
		//判断预订房型是否满房
		if( true == this.hotelCheckOrderService.isRoomFull(params)){
		...
		if (PayMethod.PRE_PAY.equals(hotelOrderFromBean.getPayMethod())
				&& ChannelType.CHANNEL_CTS == hotelOrderFromBean
						.getRoomChannel() && hotelOrderFromBean.isFlagCtsHK()) {
			...
			priceLis = hotelManageWeb.queryPriceForWebHK(hotelOrderFromBean
					.getHotelId(), hotelOrderFromBean.getRoomTypeId(),
					hotelOrderFromBean.getChildRoomTypeId(), checkIn, DateUtil
							.getDate(checkOut, -1));
			List<Date> liDate = hotelManageWeb.queryQtyForWebHK(
					hotelOrderFromBean.getHotelId(), hotelOrderFromBean
							.getRoomTypeId(), checkIn, checkOut);
			double reTotal = 0.0;
			log
					.info("HotelCompleteAction =======CTS=====hotelOrderFromBean.getPriceNum()-----"
							+ hotelOrderFromBean.getPriceNum());
			if (null != priceLis && days <= priceLis.size()
				...
				for (int k = 0; k < priceLis.size(); k++) {
					QueryHotelForWebSaleItems it = (QueryHotelForWebSaleItems) priceLis
							.get(k);
					saleBuffer.append(it.getSalePrice() + "#");
					baseBuffer.append(it.getBasePrice() + "#");
					reTotal += it.getSalePrice();
					log
							.info("HotelCompleteAction ======CTS======priceLis[k].salePrice:"
									+ it.getSalePrice());
					log
							.info("HotelCompleteAction ======CTS======priceLis[k].basePrice:"
									+ it.getBasePrice());
				}
				hkPrices = saleBuffer.toString();
				hkBasePrices = baseBuffer.toString();
				// 需要比较原币种(即港币)价格是否相等，如果原币种价格相等，则表示价格没有发生变化；如果不等，则需要打上已变化标记
				//由于priceNum已经变化，要做相应的变化 add by diandian.hou 2010-9-24
				if(PayMethod.PRE_PAY.equals(hotelOrderFromBean.getPayMethod())){
					reTotal*=CurrencyBean.getRateToHKD(hotelOrderFromBean.getCurrency());
					reTotal=CurrencyBean.numSaveInDecimal(reTotal, 0);
				}
				if (0 != Double.compare(hotelOrderFromBean.getPriceNum()
						.doubleValue(), reTotal)) {
				...
				for (int i = 0; i < days; i++) {
					if (!chkCanBook[i]) {
						...
						sbCanBook.append(DateUtil.toStringByFormat(DateUtil
								.getDate(checkIn, i), "MM-dd"));
					}
				}
		...
		fillOrderBaseInfo();
		// 设置订单来源 add by diandian.hou
		setOrderSourceForFantiWeb(order, OrderSource.FAN_TI_NET);
		// 更新会员的常入住人和常联系人
		...
		/* 预付流程 */
		if ((PayMethod.PRE_PAY.equals(hotelOrderFromBean.getPayMethod()) || hotelOrderFromBean
				.isPayToPrepay())
				&& ChannelType.CHANNEL_CTS != hotelOrderFromBean
						.getRoomChannel()) {
			...
			if(!order.isManualOrder() && hotelOrderFromBean.isChannelToWith() ) {
				...
				log
						.info("HotelCompleteAction======chinaonline======pre_pay ----start==============");
			...
			//如果是预付走网上支付流程，如果全部是支付和代金券，不走网上支付
			if(hotelOrderFromBean.getActuralAmount()>0.01D){
				return "payOnline";
			}
		}
		/* 面付流程 */
		...
		/* 中旅 预付流程 */
			...
			// 走PPS网上支付流程 add by shengwei.zuo 2009-11-23
			return toForward;

		}
		/* 畅联 流程 */
		// else if(!order.isManualOrder() && 0 <
		// hotelOrderFromBean.getRoomChannel()
		// && hotelOrderFromBean.getRoomChannel() ==
		// ChannelType.CHANNEL_CHINAONLINE) {
		// //如果是预付或者是面付且需要担保，则直接下到畅联
		// if(PayMethod.PRE_PAY.equals(hotelOrderFromBean.getPayMethod())
		// || (PayMethod.PAY.equals(hotelOrderFromBean.getPayMethod())
		// && hotelOrderFromBean.isNeedAssure() && 0 !=
		// Double.compare(hotelOrderFromBean.getOrignalSuretyPrice(), 0.0))){
		// String hdlOrderPreStr = saveHDLOrderInfo();
		// //返回的字符串如果不为空，则证明有问题 说明下单到畅联失败。
		// if(StringUtil.isValidStr(hdlOrderPreStr)){
		// return super.forwardError(hdlOrderPreStr);
		...
		int nights = DateUtil.compare(order.getCheckinDate(), order
				.getCheckoutDate());
		...
		try {
			...
			String city_name = InitServlet.cityObj.get(hotelOrderFromBean
					.getCityId());
			// System.out.println("*** city_name = " + city_name + " ; id= " +
			// hotelOrderFromBean.getCityId()
	...
	private void savePayOrderInfo() {
		...
		if (hotelOrderFromBean.isNeedAssure()
				&& 0 != Double.compare(hotelOrderFromBean
						.getOrignalSuretyPrice(), 0.0)) {
			// 去掉小数点后的金额；进行四舍五入
			...
			/*
			 * 获取订单配额, 如果获取成功则同时填充orderItems deduct[0]
			 * 作方法调用返回值用，0为调用成功;deduct[1] 作是否扣配额用 0则什么也不做；-2为已扣配额（发生异常时返配额用）
			 */
			int[] deduct = new int[2];
			deduct = hotelOrderCompleteService.fillOrderItems(order,
					hotelOrderFromBean);
			// 成功扣配额后，增加日志
			if (0 == deduct[0]) {
				...
				handleLog.setContent("面付担保单，扣配额成功");
				handleLog.setModifierName(FANTINET);
				handleLog.setOrder(order);
				order.getLogList().add(handleLog);
			}

			// 网站优化需求-开放简/繁体站所有非直联国内酒店网上预订即时确认功能 add by shengwei.zuo
			// 2010-2-1 begin

			// 网站优化需求-开放简/繁体站所有非直联国内酒店网上预订即时确认功能 add by shengwei.zuo
			// 2010-2-1 end
			// 扣配额后，发送担保单邮件给hk组 add by diandian.hou
			fantiWebTransactService.sendNeedAssureOrderMailToHK(order);
		...
		// 如果不需要担保，则直接生成订单，不扣配额，订单状态为“已提交中台”
			...
			if (hotelOrderFromBean.getRoomChannel() > 0) {
				...
				log
						.info("HotelCompleteAction.savePayOrderInfo()============pay  chinaonline --kou quota start ==============");
				...
				if (0 == deduct[0]) {
					log
							.info("HotelCompleteAction.savePayOrderInfo()============pay  chinaonline --kou quota success ==============");
					// 记录操作日志
					OrHandleLog handleLog = new OrHandleLog();
					handleLog.setModifiedTime(new Date());
			...
			if (hotelOrderFromBean.getRoomChannel() < 1) {
				...
				// 成功扣配额后，增加日志
					log
							.info("HotelCompleteAction.savePayOrderInfo()============pay order --kou quota success ==============");
					// 记录操作日志
					OrHandleLog handleLog = new OrHandleLog();
					handleLog.setModifiedTime(new Date());
					 if(-1 == deduct[0]){
			            	handleLog.setContent("面付非担保单,扣呼出配额或扣配额失败");
			            }else if( 0 == deduct[0]){
				...
				// 发送传真给酒店
				String sendHotelResult = orderImmedConfirmService
						.sendImmedConfirmToHotel(order,member);
		...
		fantiWebTransactService.synchroSumPriceToOrder(order);

		// 变价通知 add by shengwei.zuo 2010-6-4
		if (order.getSum() != hotelOrderFromBean.getPriceNum()
	...
	/**
	 * 封装订单基本数据(预付) 必须先调用fillOrderBaseInfo() 中旅/直联等PPS网上支付方式的逻辑不在本方法中判断
	 */
	...
	private String savePrepayOrderInfo() {
		...
		if (hotelOrderFromBean.isUseUlmPoint()) {
			try {
				...
				double ulmPointValue = Double.parseDouble(hotelOrderFromBean
						.getUlmPoint()) / 100;
				boolean isDeducted = hotelOrderCompleteService.deductUlmPoint(
						order, ulmPointValue, hotelOrderFromBean
								.isUseUlmPoint());
				if (!isDeducted) {
				...
				log.error(
								"ErrCord:75211 HotelCompleteAction.fillPrepayOrderInfo() deduct member point is error: ",
								e);
			}
		}
		/** 如果使用了积分，则调用积分接口 end * */
		...
		if ((hotelOrderFromBean.isUseUlmPoint() || hotelOrderFromBean
				.isUsedCoupon())
				&& 0.001D > hotelOrderFromBean.getActuralAmount()) {
			log.info("==========HotelCompleteAction.fillPrepayOrderInfo() 积分或代金券累加是全额支付 code:5652 orderCD:"
							+ orderCD + "=============");
			order.setOrderState(OrderState.SUBMIT_TO_MID);
			// //扣配额
			...
			if (0 == deduct[0]
					&& !StringUtil.isValidStr(hotelOrderFromBean
							.getSpecialRequest())) {
		 ...
		 * by chenjiajie 2009-11-06 香港信用卡方式： add by diandian.hou
		 */
		// 信用卡支付 (扣配额)
		else if (OrderPayType.CARD_HK_PAY == hotelOrderFromBean
				.getOrderPayType()) {
			log
					.info("==========HotelCompleteAction.fillPrepayOrderInfo() 信用卡支付 code:5653 orderCD:"
							+ orderCD + "=============");
			// TODO:是否使用这个变量判断
			if (isInvoice) {
				log
						.info("==========HotelCompleteAction.fillPrepayOrderInfo() 信用卡支付->需要发票 code:565 orderCD:"
								+ orderCD + "=============");
				OrFulfillment fulfill = new OrFulfillment();
				MyBeanUtil.copyProperties(fulfill, params);
				String invoiceDeliveryAddress = (String) params
						.get("invoiceDeliveryAddress");
			...
			// 如果是香港信用卡支付，做相应的转换 add by diandian.hou
			int onlinePaytype = PrepayType.RMHKEPSInt;
			...
			payment.setCreator("HHKWEB");
			payment.setOperator("HHKWEB");
			...
			if (0 == deduct[0]) {
				...
				OrHandleLog handleLog = new OrHandleLog();
				handleLog.setModifiedTime(new Date());
				handleLog.setContent("会员使用香港信用卡支付，扣配额成功");
				handleLog.setModifierName(FANTINET);
				handleLog.setOrder(order);
				order.getLogList().add(handleLog);
		...
		// 营业部付款 (不扣配额)
		else if (OrderPayType.COUNTER_PAY == hotelOrderFromBean
				.getOrderPayType()) {
			log
					.info("==========HotelCompleteAction.fillPrepayOrderInfo() 现付 code:5654 orderCD:"
							+ orderCD + "=============");
			// 设置配送信息
			OrFulfillment fulfill = new OrFulfillment();
			MyBeanUtil.copyProperties(fulfill, params);
			...
			// 配送日期为空bug
			if (null == fulfill.getDeliveryDate()
					&& null != fulfill.getFulfillPayDate()) {
		...
		else if (OrderPayType.PPS_HK_PAY==hotelOrderFromBean.getOrderPayType()){
			log
					.info("==========HotelCompleteAction.fillPrepayOrderInfo() PPS网上支付 code:56fff53 orderCD:"
							+ orderCD + "=============");
			// TODO:是否使用这个变量判断
			order.setOrderState(OrderState.NOT_SUBMIT);

			// 如果是pps支付，或者是香港信用卡做相应的转换 add by diandian.hou
			//onlinePaytype 和prepaytype大多是不一样的，这里没有新建prepayType变量是有考虑
		    // add by diandian.hou
			int	onlinePayType = PrepayType.RMHKPPSInt;
			order.setPrepayType(onlinePayType);
			...
			payment.setOrder(order);
			order.getPaymentList().add(payment);
			// 订单提交中台(避免出现给酒店发传真了，不经过中台直接撤单的情况 begin)
			order.setOrderState(OrderState.SUBMIT_TO_MID);
		    order.setToMidTime(DateUtil.getSystemDate());
		    // 订单提交中台(避免出现给酒店发传真了，不经过中台直接撤单的情况 end)
			int[] deduct = new int[2];
			deduct = hotelOrderCompleteService.fillOrderItems(order,
					hotelOrderFromBean);
			// 成功扣配额后，增加日志
	...
	private void fillBenefitInfo() {
		...
		int benefitTotalAmount = benefitService.calculateBenefitAmount(String
				.valueOf(order.getChildRoomTypeId()), order.getCheckinDate(),
	...
	private String saveDirectOrderInfo() {
		...
		if (ChannelType.CHANNEL_CTS == hotelOrderFromBean.getRoomChannel()
			...
			// 如果是pps支付，做相应的转换 add by diandian.hou
			int onlinePayType = hotelOrderFromBean.getOrderPayType();
			if (onlinePayType == OrderPayType.PPS_HK_PAY) {
				onlinePayType = PrepayType.RMHKPPSInt;
			}
			if (onlinePayType == OrderPayType.CARD_HK_PAY) {
				onlinePayType = PrepayType.RMHKEPSInt;
			}
			order.setPrepayType(onlinePayType);
	...
	public String saveHDLOrderInfo() {
		...
		if (0 < hotelOrderFromBean.getRoomChannel()) {
			...
			addExRoomOrderRequest.setChannelType(hotelOrderFromBean
					.getRoomChannel());
			...
			exRoomOrder.setCheckindate(DateUtil.toStringByFormat(order
					.getCheckinDate(), "yyyy-MM-dd"));
			...
			if (order.getPayMethod().equals(PayMethod.PRE_PAY)) {
				...
				exRoomOrder.setTotalamount(Double.valueOf(order.getSum())
						.floatValue());
			}
			exRoomOrder.setLocaltotalamout(Double.valueOf(order.getSumRmb())
					.floatValue());

			exRoomOrder.setExchangerate(Double.valueOf(order.getRateId())
					.floatValue());
			...
			log
					.info("HotelCompleteAction=============================saveHDLOrderInfo===order.getRemark().getHotelRemark() start");
			...
			exRoomOrder.setIsguarantee(String.valueOf((order
					.getIsCreditAssured())));
			...
			log
					.info("HotelCompleteAction=============================saveHDLOrderInfo===orderCD : "
							+ orderCD);
			log
					.info("HotelCompleteAction=============================saveHDLOrderInfo===exRoomOrder setObject over ");

			List<OrOrderItem> orderItemList = order.getOrderItems();
			log
					.info("HotelCompleteAction=================saveHDLOrderInfo===orderItemList start ");
			for (Iterator itr = orderItemList.iterator(); itr.hasNext();) {
				...
				mgExOrderItem.setCheckindate(DateUtil.toStringByFormat(
						orderItem.getNight(), "yyyy-MM-dd"));
				mgExOrderItem.setCheckoutdate(DateUtil.toStringByFormat(
						orderItem.getNight(), "yyyy-MM-dd"));
				mgExOrderItem.setGuests(order.getFellowNames());
				if (null != orderItem.getRoomNo()) {
					mgExOrderItem.setRoomno(Integer.valueOf(orderItem
							.getRoomNo()));
				}
				if (order.getPayMethod().equals(PayMethod.PRE_PAY)) {
					exRoomOrder.setTotalamount(exRoomOrder.getTotalamount()
							+ Double.valueOf(orderItem.getBasePrice())
									.floatValue());
				}
				mgExOrderItem.setSum(Double.valueOf(orderItem.getBasePrice())
						.floatValue());
				...
				mgExOrderItem.setSum(Double.valueOf(order.getSum())
						.floatValue());
				mgExOrderItem.setNoteresult(orderItem.getNoteResult());
				mgExOrderItem.setOrderid(order.getID());
				mgExOrderItem.setOrderstate(String.valueOf(order
						.getOrderState()));
				...
				if (0 == DateUtil.compare(orderItem.getNight(),
						hotelOrderFromBean.getCheckinDate())) {
					float colFirstDayPrice = Double.valueOf(
							orderItem.getSalePrice()).floatValue();
					log
							.info("HotelCompleteAction===========saveHDLOrderInfo=========orderItemList========colFirstDayPrice :"
									+ colFirstDayPrice);
					exRoomOrder.setFirstDayPrice(colFirstDayPrice);
				}
			}
			log
					.info("HotelCompleteAction=================saveHDLOrderInfo===orderItemList end ");
			...
			try {
				addExRoomOrderResponse = hdlService
						.addExRoomOrder(addExRoomOrderRequest);
			} catch (Exception e) {
                ...
                if (null != addExRoomOrderResponse) {
                    ...
                    if (1 == mgresult.getValue()) {
                    	...
                    	log.error("=======HotelCompleteAction.saveHDLOrderInfo()====== new order failed! reason :  "+ mgresult.getMessage());
                        // 直联订单失败的原因是合作方系统原因,
                        order.setCancelReason(88);
                	 ...
                	 log.error("============HotelCompleteAction.saveHDLOrderInfo() new order failed resaon: addExRoomOrderResponse is null !");
                	 order.setCancelReason(88);
	...
	private String processPaymentMethod(OrOrder order) {
		...
		log.info("Success... HotelCompleteAction.processPaymentMethod(:"
				...
				+ ") save success and orderService.getOrderCDByID(orderCD="
				+ orderCD + ") success");
		// }
		if (null != orderId && null == order.getID()) {
			order.setID(new Long(orderId.toString()));
	...
	private void setOrderSourceForFantiWeb(OrOrder order, String scoure) {
		order.setSource(scoure);
	}
