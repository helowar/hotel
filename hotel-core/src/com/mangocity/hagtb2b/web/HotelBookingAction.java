...
//  原始查询条件封闭类
  ...
  //日历控件在前台的显示
	...
	//从cookie中获取是否为嵌套版用户，如果是嵌套版用户屏蔽用户须知
	private String qianTaoFlag;
	
	private static String QIAN_TAO_FLAG="qianTaoFlag";

	 /**
     * 判断是否是底价支付的订单 add by xuyiwen 2010-12-8
     */
    private String judgeMinPrice;
	...
	public String getJudgeMinPrice() {
		return judgeMinPrice;
	}

	public void setJudgeMinPrice(String judgeMinPrice) {
		this.judgeMinPrice = judgeMinPrice;
	}
	...
	public String execute() {
		...
		qianTaoFlag = CookieUtils.getCookieValue(request, QIAN_TAO_FLAG);
...
//        }
        ...
        if(hotelOrderFromBean.isChannelToWith()){
			...
			//B2B订单填写的日历界面 如果为B2B加幅订单显示价格为底价+加幅 add by xuyiwen  2010-12-10 begin
			if(null!=judgeMinPrice){
				if("true".equals(judgeMinPrice.trim())){
					//step1 得到入住日期
					//Date inDate = hotelOrderFromBean.getCheckinDate(); 不应当是入住日期，而应当是当前日期
					Date inDate = new Date();
					//step 2 得到下个月的最后一天
					Date nextMonthEndDate =DateUtil.toDateByFormat(DateUtil.getEndDate(DateUtil.getDateByMonth(inDate, 1)),"yyyy-MM-dd");
					
					//step 3 得到加幅价格
					List<HtlB2bIncrease> increasePrices = htlB2bService.queryIncreasePrice(hotelOrderFromBean.getHotelId(), hotelOrderFromBean.getChildRoomTypeId(), inDate, nextMonthEndDate);
				
					//step 4 修改价格
					calendarExtenderLstOne = modifyPrice(increasePrices,calendarExtenderLstOne);
					calendarExtenderLst = modifyPrice(increasePrices,calendarExtenderLst);
				}
			}
			//end 
	        ...
	        if (hotelOrderFromBean.getPayMethod().equals("pre_pay")
	            ...
	            if(null !=priceLis){
	            	// 这里将调取接口取得的价格写入数据库 add by xuyiwen 2011-1-18
		             hkManage.updateB2BCTSHotelPrice(priceLis, hotelOrderFromBean.getHotelId(), hotelOrderFromBean.getChildRoomTypeId(),hotelOrderFromBean.getRoomTypeId());
		             
	            	//中科酒店重新调取接口取价格后 需要对其重新判断更改价格 B2B预售底价 add by xuyiwen 2011-1-11
	            	priceLis = htlB2bService.modifyIncreaePrice(priceLis, hotelOrderFromBean.getHotelId(), hotelOrderFromBean.getChildRoomTypeId(), hotelOrderFromBean.getCheckinDate(), hotelOrderFromBean.getCheckoutDate());
	            	log.info("HotelReBookingAction==========CTS===========B2B预售底价 ");
	            	//end
	            }
...
//	                }
    ...
    /**
     * 修改日历价格 add by xuyiwen 2010-12-14
     * @param htlB2bIncreases
     * @param htlCalendarRowsBeans
     * @return
     */
    private List<HtlCalendarRowsBean> modifyPrice(List<HtlB2bIncrease> htlB2bIncreases,List<HtlCalendarRowsBean> htlCalendarRowsBeans){
    	if(htlB2bIncreases.size()<= 0 ||htlCalendarRowsBeans.size()<=0) return htlCalendarRowsBeans;
    		for(HtlCalendarRowsBean htlCalendarRowsBean : htlCalendarRowsBeans){
    			List<HtlCalendarColsPriceBean> htlCalendarColsPriceBeans =htlCalendarRowsBean.getLstColsPrice();
    			for(HtlCalendarColsPriceBean htlCalendarColsPriceBean : htlCalendarColsPriceBeans){
    				for(HtlB2bIncrease htlB2bIncrease: htlB2bIncreases){
    				//判断两个日期是否相等 精确到天
    				if(DateUtil.dateToString(htlB2bIncrease.getAbleSaleDate()).equals(DateUtil.dateToString(htlCalendarColsPriceBean.getCurrDate()))){
    					htlCalendarColsPriceBean.setDayPrice(Math.ceil(htlB2bIncrease.getIncreasePrice()));
    					htlCalendarColsPriceBean.setDayPriceStr(getChangePriceStyleOne(Math.ceil(htlB2bIncrease.getIncreasePrice())));
    				}
    			}
    		}
    	}
    	
    	return htlCalendarRowsBeans;
    }
	...
	public String getQianTaoFlag() {
		return qianTaoFlag;
	}

	public void setQianTaoFlag(String qianTaoFlag) {
		this.qianTaoFlag = qianTaoFlag;
	}
