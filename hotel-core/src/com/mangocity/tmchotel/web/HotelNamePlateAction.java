...
public class HotelNamePlateAction extends GenericCCAction{
	...
	//酒店SEO 页面title的显示   add by shengwei.zuo 2010-3-5
...
//	存放我浏览过的酒店的hotelId、hotelName
	...
	public String execute(){
		...
		//SEO 品牌 add by shengwei.zuo 2010-3-9
        if(StringUtil.isValidStr(parentId)){
        	if(StringUtil.isValidStr(jiudian)){
        		//根据城市名称和行政区编码，查询出行政区名称
        		String districtName = hotelManageWeb.queryParentNaHtlInfo(cityCode, parentId);
        		if(StringUtil.isValidStr(districtName)){
        			titleTexts =  districtName ;
        		}
        	}
        }
	...
	public void setTitleTexts(String titleTexts) {
