create or replace procedure sp_hotelsort(p_startdate    in varchar2,
                                         ...
                                         p_pagesize     in int,
                                         p_currpage     in int,
										 p_geoPositionId  in number, --地标位置的ID,根据酒店到该地理位置的距离进行排序
  ...
  issaleonnet_sql    varchar2(1024); --酒店是否在外网售卖
...
begin
 ...
 /* if l_count = 1 then
  ...
  incontractdate_sql := 'select hc.hotel_id
                      ...
                      where hc.hotel_id = hq.hotelid and hc.begin_date <= to_date(:startDate,''yyyy-mm-dd'')
                      and hc.end_date >= to_date(:endDate,''yyyy-mm-dd'')-1 ';
  stopsell_sql       := 'select hhe.hotel_id
                  from htl_hotel_ext hhe
         where ((to_date(:startDate,''yyyy-mm-dd'') between hhe.stopsell_begin_date and hhe.stopsell_end_date)
                 or(to_date(:endDate,''yyyy-mm-dd'')-1 between hhe.stopsell_begin_date and hhe.stopsell_end_date))
            and hhe.hotel_id = hq.hotelid and hhe.ISSTOPSELL = ''1'' ';
   issaleonnet_sql := 'select hhe.hotel_id from htl_hotel_ext hhe
                      where hhe.hotel_id = hq.hotelid and hhe.ISOUTERNETSALE = ''1'' ';

  --查询只读表,得到条款,价格,开关房,房态信息,并计算出不可订的酒店
  if upper(p_citycode) = 'PEK' or upper(p_citycode) = 'SHA' or
     upper(p_citycode) = 'CAN' or upper(p_citycode) = 'SZX' or
     upper(p_citycode) = 'HKG' or upper(p_citycode) = 'HGH' or
     upper(p_citycode) = 'CTU' or upper(p_citycode) = 'NKG' then
    ...
    begin
      select ha.AREACODE
        into l_AREACODE
        from htl_area ha
       where ha.CITYCODE = upper(p_citycode)
         and rownum = 1;
      if l_AREACODE = 'HDQ' or l_AREACODE = 'HBQ' then
        tablename := 'HTLQUERY_' || l_AREACODE;
      else
  ...
  temp_sql := ' select count(*) from ( select hq.hotelid
                    from ' || tablename || ' hq
                where hq.active = 1 and hq.hotelid in ( select hotelid from tmp_hotel_sort )
                and abledate >= to_date(:startDate,''yyyy-mm-dd'')
                and abledate < to_date(:endDate,''yyyy-mm-dd'')
                and exists (' || incontractdate_sql || ')
                and exists (' || issaleonnet_sql || ')
                and not exists (' || stopsell_sql || ') 
                         group by hq.hotelid ) d';
  execute immediate temp_sql
    into hotelcount
    using p_startdate, p_enddate, p_startdate, p_enddate, p_startdate, p_enddate;
  ...
  --条款
  item_sql := ' min((case when (hq.abledate = to_date(:startDate,''yyyy-mm-dd'')
                           and trunc(sysdate) between
                               nvl(hq.bookstartdate, trunc(sysdate) - 3000) and
                               nvl(hq.bookenddate, trunc(sysdate) + 3000)
                           and :booktime between
                               nvl(hq.morningtime, ''00:00'') and
                               nvl(hq.eveningtime, ''23:59'')
                           and :days >= nvl(hq.continueday, 0)
                           and (nvl(hq.restrict_in,0) = 0 or hq.restrict_in = :days ))
                           or hq.abledate <> to_date(:startDate,''yyyy-mm-dd'')
                           then 1
                           else 0
                           end)*  ';
  ...
  notitem_sql := ' (case
                            when (nvl(hq.closeflag, ''K'') != ''G'' and
                             hq.saleprice > 0 and hq.saleprice < 900000 and 
                                 hq.hasbook = 1) then
                             1
                             else
                             0
  ...
  temp_sql := ' select hq.hotelid,hq.commodityid,hq.bedtype,hq.paymethod,' ||
              item_sql || notitem_sql || ' count(*) as total, min((case when hq.hasbook = 1 and nvl(hq.closeflag, ''K'') != ''G'' then decode(hq.saleprice,0,999999,hq.saleprice) else 999999 end)) as minprice
                    from ' || tablename || ' hq
                where hq.active = 1 and hq.hotelid in ( select hotelid from tmp_hotel_sort )
                and abledate >= to_date(:startDate,''yyyy-mm-dd'')
                and abledate < to_date(:endDate,''yyyy-mm-dd'')
  ...
  temp_sql := 'select c1.hotelid,c1.canbook,c1.minprice,
                            ...
                            max(c.canbook*(case when c.total >= :days then 1 else 0 end)) as canbook,
  ...
  if p_geoPositionId is not null then
    temp_sql := 'select d.hotelid,d.canbook, FUN_GET_DISTANCE_TO_POSITION('||p_geoPositionId||',d.hotelid) as distance from (' || temp_sql ||
              ') d order by d.canbook desc, distance ';
  else
  ...
  temp_sql := 'select indexNo,hotelid from ( select rownum indexNo,e.hotelid,e.canbook from
              (' || temp_sql ||
              ') e) where indexNo > :index1 and indexNo <= :index2';
  ...
  open hotelIdcursor for temp_sql
    using days, p_startdate, booktime, days, days, p_startdate, p_startdate, p_enddate, p_startdate, p_enddate, p_startdate, p_enddate,((currpage - 1) *
                                                                                                                                        pagesize),(currpage *
                                                                                                                                                   pagesize);
...
exception
  when others then
    ...
    insert into htl_error_log
      (id, operate_name, operate_type, error_msg, gwt_created)
    values
      (SEQ_HTL_ERROR_LOG.Nextval,
       'htl_ii.sp_hotelsort',
       5,
       l_errmsg,
       sysdate);
