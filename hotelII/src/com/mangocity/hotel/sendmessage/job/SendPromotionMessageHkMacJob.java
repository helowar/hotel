...
public class SendPromotionMessageHkMacJob{
	...
	private static final MyLog log = MyLog.getLogger(SendPromotionMessageHkMacJob.class);

	protected void execute(){
		OrParam orParam = new OrParam();
		try {
			log.info("发送港澳暑假促销促销信息定时器begin！ ");
			...
			if(nextSeq.equals(orParam.getValue())){			
				...
				log.info("发送港澳暑假促销信息定时器在执行中");
				return;
			}

			
		} catch (Exception e) {
			log.error("发送港澳暑假促销促销信息定时器", e);
		}
		
		finally{
			log.info("发送港澳暑假促销促销信息定时器 end！ ");
		}
	}
